...
The total precipitation in 2018 is `r sum(pull(precipitation_2018, total))`.The median number of sports balls in a dumpster in 2017 is `r median(pull(filter(Mr_Trash_Wheel_data, year == 2017), sports_balls))`. 


In Mr_Trash_Wheel_data, there are `r nrow(Mr_Trash_Wheel_data)`rows and `r ncol(Mr_Trash_Wheel_data)`columns, and the key variable is dumpster, which identifies each row.

In combination_precipitation_data,there are `r nrow(Mr_Trash_Wheel_data)`rows and `r ncol(Mr_Trash_Wheel_data)`columns, and the key variable is month, which is the link between precipitation 2017 data and precipitation 2018 data. 

In precipitation_2017 data, there are `r nrow(precipitation_2017)`rows and `r ncol(precipitation_2017)`columns.The mean  is `r mean(pull(precipitation_2017, total))`.The key variable is month, because the month identifies each row.


In the precipitation_2018 data, there are `r nrow(precipitation_2018)`rows and `r ncol(precipitation_2018)`columns. The mean is `r mean(pull(precipitation_2018, total))`.The key variable is month, because the month identifies each row.
